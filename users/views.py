    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if user:
                ...
                login(request, user)
                return Response({'token': token.key, 'user_id': user.id,'user_type': user.user_type,'image_url':user.image_url,'is_premium':user.is_premium})
            else:
...
class SSLCommerzInitiatePayment(APIView):
    ...
    def post(self, request):
        ...
        post_body['fail_url'] = "http://127.0.0.1:5501/profile.html"
        post_body['cancel_url'] = "http://127.0.0.1:5501/profile.html"
